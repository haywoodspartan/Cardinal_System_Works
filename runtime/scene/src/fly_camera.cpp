...
void FlyCamera::sync_from(const Camera& cam) noexcept {
    ...
    pitch_ = std::asin(std::max(-1.0f, std::min(1.0f, fwd.y)));
    yaw_   = std::atan2(fwd.x, -fwd.z);   // matches +Z back convention
    initialised_ = true;
}
...
void FlyCamera::tick(Camera& cam, const FlyInput& in, float dt) noexcept {
    ...
    if (std::abs(in.scroll) > 0.001f) {
        const float scale = std::pow(scroll_speed_scale, in.scroll);
        speed = std::clamp(speed * scale, speed_min, speed_max);
    }
    ...
    if (in.look) {
        ...
        pitch_  = std::clamp(pitch_, min_pitch_rad, max_pitch_rad);
    }

...
static void ImGui_ImplVulkan_CreateWindow(ImGuiViewport* viewport)
{
    ...
    // Select Present Mode
    // Cardinal patch: prefer IMMEDIATE over MAILBOX. The original ordering
    // (MAILBOX first) caps each torn-out panel to display refresh rate via
    // DWM composition — which serialises the entire main loop to 60 FPS
    // when ANY secondary window is open (the same DWM-cap bug we fixed for
    // the main D3D12 swapchain by setting DXGI_PRESENT_ALLOW_TEARING).
    // Upstream's own FIXME-VULKAN comment notes "MAILBOX halves framerate
    // with a second window"; IMMEDIATE allows tearing and lets each
    // secondary swapchain run as fast as the GPU can deliver, matching
    // what ImGui_ImplDX12 now does via DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING.
...
static void ImGui_ImplVulkan_RenderWindow(ImGuiViewport* viewport, void*)
{
    ...
    ImGui_ImplVulkanH_FrameSemaphores* fsd = &wd->FrameSemaphores[wd->SemaphoreIndex];
    {
        ...
        for (;;)
        {
            ...
            // Cardinal patch: upstream loops forever on real errors (e.g.
            // DEVICE_LOST after a faulty previous submit), which spams the
            // CheckVkResultFn thousands of times per frame. Report the
            // result, mark the secondary swapchain for rebuild next frame,
            // and abandon this frame's render so the rest of the editor
            // keeps ticking.
            check_vk_result(err);
            vd->SwapChainNeedRebuild = true;
            return;

...
#include <d3d12.h>
#include <dxgi1_4.h>
#include <dxgi1_5.h>     // Cardinal patch — IDXGIFactory5 for tearing probe
#include <d3dcompiler.h>

// ----- Cardinal patch: DXGI tearing support cache --------------------------
// Probed once on the first ImGui_ImplDX12_CreateWindow call; consumed by
// ResizeBuffers + Present in the multi-viewport secondary-window path so
// torn-out editor panels don't get capped at the display refresh rate.
namespace { int g_CardinalTearingProbed = 0; BOOL g_CardinalTearingSupported = FALSE; }
#ifdef _MSC_VER
...
struct ImGui_ImplDX12_FrameContext
{
    ...
    // Cardinal patch: per-slot fence value so SwapBuffers can avoid the
    // CPU-side spin after Present. We only wait for THIS context's last
    // submitted fence value when its slot is about to be reused; the GPU
    // is otherwise free to pipeline numFramesInFlight frames per window.
    UINT64                          CardinalFenceValue;
};
...
struct ImGui_ImplDX12_ViewportData
{
    ...
    ImGui_ImplDX12_ViewportData(UINT num_frames_in_flight)
    {
        ...
        for (UINT i = 0; i < NumFramesInFlight; ++i)
        {
            ...
            FrameCtx[i].CardinalFenceValue = 0;
...
static void ImGui_ImplDX12_CreateWindow(ImGuiViewport* viewport)
{
    ...
    //
    // Cardinal patch: probe DXGI tearing support once on first window,
    // cache in TU statics so ResizeBuffers + Present can use the same flag.
    if (g_CardinalTearingProbed == 0) {
        IDXGIFactory4* probe_factory = nullptr;
        if (::CreateDXGIFactory1(IID_PPV_ARGS(&probe_factory)) == S_OK && probe_factory)
        {
            IDXGIFactory5* f5 = nullptr;
            if (probe_factory->QueryInterface(IID_PPV_ARGS(&f5)) == S_OK && f5)
            {
                f5->CheckFeatureSupport(
                    DXGI_FEATURE_PRESENT_ALLOW_TEARING,
                    &g_CardinalTearingSupported,
                    sizeof(g_CardinalTearingSupported));
                f5->Release();
            }
            probe_factory->Release();
        }
        g_CardinalTearingProbed = 1;
    }
    ...
    sd1.Stereo = FALSE;
    sd1.Flags = g_CardinalTearingSupported
...
static void ImGui_ImplDX12_SetWindowSize(ImGuiViewport* viewport, ImVec2 size)
{
    ...
    if (vd->SwapChain)
    {
        ...
        // Cardinal patch: pass the tearing flag here too — DXGI requires
        // the flag set passed to ResizeBuffers to match the flags the
        // swapchain was created with, otherwise Present(0, TEARING) fails.
        const UINT _resize_flags = g_CardinalTearingSupported
                                 ? DXGI_SWAP_CHAIN_FLAG_ALLOW_TEARING : 0u;
        vd->SwapChain->ResizeBuffers(0, (UINT)size.x, (UINT)size.y, DXGI_FORMAT_UNKNOWN, _resize_flags);
        for (UINT i = 0; i < bd->numFramesInFlight; i++)
        {
            vd->SwapChain->GetBuffer(i, IID_PPV_ARGS(&back_buffer));
            bd->pd3dDevice->CreateRenderTargetView(back_buffer, nullptr, vd->FrameCtx[i].RenderTargetCpuDescriptors);
...
static void ImGui_ImplDX12_RenderWindow(ImGuiViewport* viewport, void*)
{
    ...
    // Cardinal patch: only block the CPU when this specific frame slot's
    // last submission hasn't completed yet. With NumFramesInFlight = 3 we
    // get up to 3 frames of overlap per window — vs. the upstream code
    // which spun in SwapBuffers after every Present and serialised the
    // whole pipeline.
    if (frame_context->CardinalFenceValue != 0
        && vd->Fence->GetCompletedValue() < frame_context->CardinalFenceValue)
    {
        vd->Fence->SetEventOnCompletion(frame_context->CardinalFenceValue, vd->FenceEvent);
        ::WaitForSingleObject(vd->FenceEvent, INFINITE);
    }
    ...
    // Cardinal patch: stamp the per-slot fence value so the next time
    // this slot is reused we know what to wait on.
    frame_context->CardinalFenceValue = vd->FenceSignaledValue;
}
...
static void ImGui_ImplDX12_SwapBuffers(ImGuiViewport* viewport, void*)
{
    ...
    // Cardinal patch: pair Present with DXGI_PRESENT_ALLOW_TEARING when
    // the swapchain was created with the tearing flag. Without this the
    // secondary multi-viewport windows DWM-cap to display refresh and
    // serialise the entire main loop to 60 FPS.
    //
    // Critically — DON'T spin-wait on the fence here. The upstream code
    // did `while (Fence->GetCompletedValue() < FenceSignaledValue) yield;`
    // which fully drained the GPU pipeline per window per frame. We've
    // moved per-slot waiting into RenderWindow (only fires when the slot
    // is about to be reused), so the GPU can pipeline numFramesInFlight
    // frames of work per secondary window with no CPU stall.
    const UINT _present_flags = g_CardinalTearingSupported
                              ? DXGI_PRESENT_ALLOW_TEARING : 0u;
    vd->SwapChain->Present(0, _present_flags);
}

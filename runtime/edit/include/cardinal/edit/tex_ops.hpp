...
cardinal::vector<u8> checker(u32 w, u32 h, u32 cells_x, u32 cells_y,
                        Color a, Color b);

enum class Axis { X, Y, Diagonal };
cardinal::vector<u8> gradient_linear(u32 w, u32 h, Color a, Color b, Axis axis);

cardinal::vector<u8> noise_value (u32 w, u32 h, u32 seed = 1337,
                             float scale = 8.0f, float contrast = 1.0f);

cardinal::vector<u8> noise_fractal(u32 w, u32 h, u32 seed = 1337,
                              float base_scale = 4.0f,
                              u32   octaves = 4,
                              float persistence = 0.5f);

cardinal::vector<u8> voronoi_cells(u32 w, u32 h, u32 seed = 1337, u32 site_count = 32);

// -------- Operations -------------------------------------------------------

void to_grayscale(cardinal::vector<u8>& rgba, u32 w, u32 h);
void invert      (cardinal::vector<u8>& rgba, u32 w, u32 h);

// black_pt / white_pt in [0..1]; inputs <= black_pt map to 0, >= white_pt
// map to 255, with `gamma` applied to the rescaled middle range.
void levels(cardinal::vector<u8>& rgba, u32 w, u32 h,
...
void channel_swap(cardinal::vector<u8>& rgba, u32 w, u32 h, const char* layout);
...
inline Color get_px(const cardinal::vector<u8>& img, u32 w, u32 x, u32 y) noexcept {
    const usize i = (static_cast<usize>(y) * w + x) * 4;
    return Color{ img[i+0], img[i+1], img[i+2], img[i+3] };
}
inline void put_px(cardinal::vector<u8>& img, u32 w, u32 x, u32 y, Color c) noexcept {
    const usize i = (static_cast<usize>(y) * w + x) * 4;

...
//
// Operates on a flat cardinal::vector<Vertex>-style buffer (3 verts per triangle).
...
// ---------------------------------------------------------------------------
cardinal::vector<Vertex> make_box     (float size = 1.0f);
cardinal::vector<Vertex> make_plane   (float size = 1.0f, u32 subdivisions = 1);
cardinal::vector<Vertex> make_sphere  (float radius = 1.0f, u32 segments = 24);
cardinal::vector<Vertex> make_cylinder(float radius = 1.0f, float height = 2.0f,
                                  u32 segments = 24);
cardinal::vector<Vertex> make_cone    (float radius = 1.0f, float height = 2.0f,
                                  u32 segments = 24);
cardinal::vector<Vertex> make_torus   (float major = 1.0f, float minor = 0.3f,
                                  u32 major_segs = 32, u32 minor_segs = 16);
cardinal::vector<Vertex> make_disk    (float radius = 1.0f, u32 segments = 32);
...
cardinal::vector<Vertex> subdivide(const cardinal::vector<Vertex>& in, u32 levels = 1);
...
cardinal::vector<Vertex> recompute_normals(const cardinal::vector<Vertex>& in, bool smooth = true);
...
cardinal::vector<Vertex> tint(const cardinal::vector<Vertex>& in, const Vec3& tint);

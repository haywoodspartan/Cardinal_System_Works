// =============================================================================
// Cardinal — SIMD math: AVX (256-bit floats, 8 lanes).
//
// Compiled with /arch:AVX (MSVC) — see CMakeLists. AVX brings 256-bit
// float math; integer 256-bit ops require AVX2 (different TU). FMA3 is
// architecturally separate from AVX — first-cut here uses split mul +
// add, which the compiler MAY fuse if FMA is available at the build
// flag (it isn't with /arch:AVX alone). The AVX2 TU below explicitly
// uses FMA3 intrinsics.
// =============================================================================
#define CARDINAL_SIMD_TIER avx
// Per-op fallback demo — the AVX tier doesn't ship mat4_mul_array.
// Without FMA3 the inner-loop savings over SSE4.2's 4-wide column
// path are negligible, so we don't write a duplicate kernel. The
// dispatcher's tier walk leaves AVX's binding for that op pointing at
// SSE4.2's implementation. Other ops (vec_add_f32, dot_f32, …) DO
// ship in this tier — opt out for just the one we don't reimplement.
#define CARDINAL_SIMD_OMIT_MAT4_MUL_ARRAY 1
...
#define CARDINAL_AVX_TAIL(stmts) \
    for (; i < n; ++i) { stmts; }
...
void vec_add_f32(f32* out, const f32* a, const f32* b, usize n) {
    ...
    CARDINAL_AVX_TAIL(out[i] = a[i] + b[i]);
}
                           ...
                           const Mat4Compact& M, usize count)
{
    ...
    for (; i + 8 <= count; i += 8) {
        ...
        const __m256 ox = _mm256_add_ps(_mm256_add_ps(_mm256_mul_ps(m00, xs), _mm256_mul_ps(m04, ys)),
                                        _mm256_add_ps(_mm256_mul_ps(m08, zs), m12));
        const __m256 oy = _mm256_add_ps(_mm256_add_ps(_mm256_mul_ps(m01, xs), _mm256_mul_ps(m05, ys)),
                                        _mm256_add_ps(_mm256_mul_ps(m09, zs), m13));
        const __m256 oz = _mm256_add_ps(_mm256_add_ps(_mm256_mul_ps(m02, xs), _mm256_mul_ps(m06, ys)),
                                        _mm256_add_ps(_mm256_mul_ps(m10, zs), m14));
...
void vec3_cross_array(f32* out_xyz, const f32* a_xyz, const f32* b_xyz, usize count) {
    ...
    for (; i + 8 <= count; i += 8) {
        ...
        const __m256 ox = _mm256_sub_ps(_mm256_mul_ps(ay, bz), _mm256_mul_ps(az, by));
        const __m256 oy = _mm256_sub_ps(_mm256_mul_ps(az, bx), _mm256_mul_ps(ax, bz));
        const __m256 oz = _mm256_sub_ps(_mm256_mul_ps(ax, by), _mm256_mul_ps(ay, bx));
...
void vec3_length_array(f32* out_lens, const f32* in_xyz, usize count) {
    ...
    for (; i + 8 <= count; i += 8) {
        ...
        const __m256 ys = _mm256_set_ps(p[22], p[19], p[16], p[13], p[10], p[7], p[4], p[1]);
        const __m256 zs = _mm256_set_ps(p[23], p[20], p[17], p[14], p[11], p[8], p[5], p[2]);
        const __m256 sq = _mm256_add_ps(_mm256_add_ps(_mm256_mul_ps(xs, xs), _mm256_mul_ps(ys, ys)),
                                        _mm256_mul_ps(zs, zs));
...
// Frustum cull — 8 spheres / iter (one byte / iter). 256-bit math but
// no FMA (that's AVX2+); split mul + add. _mm256_movemask_ps packs the
// per-lane visibility down to 8 bits — a complete byte / iteration.
                          ...
                          usize count)
{
    auto test8 = [&](usize i) -> int {
        ...
        for (int p = 0; p < 6; ++p) {
            ...
            __m256 dist = _mm256_add_ps(_mm256_mul_ps(nx, vcx),
                            _mm256_add_ps(_mm256_mul_ps(ny, vcy),
                              _mm256_add_ps(_mm256_mul_ps(nz, vcz), d)));
            __m256 pass = _mm256_cmp_ps(_mm256_add_ps(dist, vr),
                                        _mm256_setzero_ps(), _CMP_GE_OQ);
            acc = _mm256_and_ps(acc, pass);
        }
        return _mm256_movemask_ps(acc);   // 8 bits
    };
...
// AABB transform — 8 AABBs / iter via 256-bit SoA. Same Arvo
// structure as SSE4.2 but with __m256; mul + add chains since AVX has
// no FMA (that's AVX2+).
...
// Frustum-vs-AABB cull — 8 AABBs / iter, no FMA.
                        ...
                        usize count)
{
    ...
    auto test8 = [&](usize i) -> int {
        ...
        for (int p = 0; p < 6; ++p) {
            ...
            __m256 dist = _mm256_add_ps(_mm256_mul_ps(nx, vpx),
                            _mm256_add_ps(_mm256_mul_ps(ny, vpy),
                              _mm256_add_ps(_mm256_mul_ps(nz, vpz), d)));
            __m256 pass = _mm256_cmp_ps(dist, _mm256_setzero_ps(), _CMP_GE_OQ);
            acc = _mm256_and_ps(acc, pass);
        }
...
// argmax — AVX path. 8 lanes / iter; mirror of min_index_f32 with
// _CMP_GT_OQ + _mm256_max_ps.
...
// 3D Morton encoding — AVX1 doesn't expose 256-bit integer shifts
// (those came with AVX2), so this tier falls back to the same 128-bit
// path the SSE4.2 tier uses. Two u64 lanes per inner iteration.
namespace {
inline __m128i mt_spread21_2x(__m128i v) noexcept {
                            ...
                            usize count)
{
    ...
    for (; i + 2 <= count; i += 2) {
        ...
        const __m128i sx = mt_spread21_2x(vx);
        const __m128i sy = mt_spread21_2x(vy);
        const __m128i sz = mt_spread21_2x(vz);
...
// argmin reduction — AVX path. 8 lanes / iter; same parallel-track
// pattern as SSE4.2. _mm256_blendv_ps handles per-lane index
// selection; horizontal reduce after the bulk loop is scalar.
...
// Ray-vs-AABB batch — 8 AABBs / iter via __m256 SoA, no FMA.
void ray_aabb_intersect_array(
    ...
    usize count)
{
    const __m256 vox = _mm256_set1_ps(ox);
    const __m256 voy = _mm256_set1_ps(oy);
    const __m256 voz = _mm256_set1_ps(oz);
    ...
    usize i = 0;
    for (; i + 8 <= count; i += 8) {
        const __m256 t0x = _mm256_mul_ps(_mm256_sub_ps(_mm256_loadu_ps(min_x + i), vox), vix);
        const __m256 t1x = _mm256_mul_ps(_mm256_sub_ps(_mm256_loadu_ps(max_x + i), vox), vix);
        const __m256 t0y = _mm256_mul_ps(_mm256_sub_ps(_mm256_loadu_ps(min_y + i), voy), viy);
        const __m256 t1y = _mm256_mul_ps(_mm256_sub_ps(_mm256_loadu_ps(max_y + i), voy), viy);
        const __m256 t0z = _mm256_mul_ps(_mm256_sub_ps(_mm256_loadu_ps(min_z + i), voz), viz);
        const __m256 t1z = _mm256_mul_ps(_mm256_sub_ps(_mm256_loadu_ps(max_z + i), voz), viz);
